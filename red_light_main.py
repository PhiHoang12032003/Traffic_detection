...
def generate_frames_red_light_new(video_path, video_id=None, violation_db_instance=None):
    """
    Generator function for streaming red light detection frames
    Args:
        video_path: Path to video file
        video_id: Database video ID (optional, for database integration)
        violation_db_instance: ViolationDatabase instance (optional, for saving violations)
    """
    # Create the buffers
    frame_pipeline = Pipeline()
    processed_pipeline = Pipeline()
    ...
    # Use violation_db passed as parameter
    violation_db = violation_db_instance
    print(f"🔍 [STREAMING DEBUG] Starting stream with video_id={video_id}")
    print(f"🔍 [STREAMING DEBUG] violation_db_instance provided: {violation_db_instance is not None}")
    
    if violation_db is not None:
        print(f"✅ [STREAMING DATABASE] ViolationDatabase instance available for video_id={video_id}")
    else:
        if video_id:
            print(f"⚠️ [STREAMING DATABASE] video_id={video_id} but violation_db_instance is None")
        else:
            print(f"⚠️ [STREAMING DATABASE] No video_id and no violation_db_instance")
    ...
    try:
        while True:
            ...
            if violating_plate_text_box_info is not None:
                ...
                # Lưu vi phạm vào database ngay khi phát hiện
                violation_count += 1
                try:
                    # Lưu ảnh vi phạm
                    os.makedirs("data_vuot_den_do", exist_ok=True)
                    ...
                    if violation_db is not None and video_id is not None:
                        try:
                            ...
                            print(f"🔍 [STREAMING DB] Saving violation to database:")
                            print(f"  - video_id={video_id}")
                            print(f"  - frame_number={frame_count}")
                            print(f"  - time_in_video={time_in_video}")
                            print(f"  - license_plate={violating_plate_text}")
                            ...
                            if v_id:
                                print(f"✅✅✅ [STREAMING DB SUCCESS] Violation saved immediately to database!")
                                print(f"✅ [CAMERA 3 - STREAMING] violation_id={v_id}, plate={violating_plate_text}")
                            else:
                                print(f"❌ [STREAMING DB ERROR] insert_red_light_violation returned None")
                                
                        except Exception as db_err:
                            print(f"❌❌❌ [STREAMING DB ERROR] Failed to save: {db_err}")
                            import traceback
                            traceback.print_exc()
                    else:
                        if not violation_db:
                            print(f"⚠️ [STREAMING DB SKIP] violation_db is None")
                        if not video_id:
                            print(f"⚠️ [STREAMING DB SKIP] video_id is None")
                
                except Exception as e:
                    print(f"Error creating PDF/saving violation {violation_count}: {e}")
                    import traceback
                    traceback.print_exc()
