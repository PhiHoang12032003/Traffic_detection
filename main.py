...
def vehicle_count(source_path, destination_path, line_start, line_end):
    # Load the pre-trained YOLO12 model
    ...
    # Create a line zone annotator object with specific thickness and text scale (thicker for better visibility)
    line_annotator = sv.LineZoneAnnotator(thickness=6,
                                          text_thickness=2,
                                          text_scale=0.7)
    ...
    for result in model.track(source=source_path, tracker='bytetrack.yaml', show=True, stream=True, agnostic_nms=True):
        ...
        # Filter the detections to only include vehicles (cars, motorcycles, trucks)
        # COCO classes: 2=car, 3=motorcycle, 7=truck
        detections = detections[(detections.class_id == 2) | (detections.class_id == 3) | (detections.class_id == 7)]
